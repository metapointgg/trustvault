...
class _FitsOperationsScreenState extends State<FitsOperationsScreen> {
  ...
  final TextEditingController _entityController =
      TextEditingController(text: 'northshore-001');
  final TextEditingController _queryController =
      TextEditingController(text: 'verified');
  ...
  Future<void> _run(Future<Map<String, dynamic>> Function() action,
      void Function(Map<String, dynamic>) assign) async {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
              'Inspect, search and index directly from the current per-entity FITS evidence container.'),
            ...
            child: ListView(
              children: [
                if (_inspectResult != null)
                  _InspectionPanel(result: _inspectResult!),
                if (_directSearchResult != null) ...[
                  const SizedBox(height: 16),
                  _SearchPanel(
                      title: 'Direct FITS search',
                      result: _directSearchResult!),
                ],
                if (_indexSearchResult != null) ...[
                  const SizedBox(height: 16),
                  _SearchPanel(
                      title: 'Index-backed FITS search',
                      result: _indexSearchResult!),
                ],
...
class _InspectionPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final hduNames =
        (result['hdu_names'] as List<dynamic>? ?? <dynamic>[]).join(', ');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
                'Results: ${result['result_count']} from container ${result['container_version_id'] ?? '-'}'),
            const SizedBox(height: 12),
            if (results.isEmpty)
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('FITS index rebuild',
                style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 8),
            _DetailRow(
                label: 'Indexed entities',
                value: '${result['indexed_entity_count']}'),
            _DetailRow(
                label: 'Skipped entities',
                value: '${result['skipped_entity_count']}'),
