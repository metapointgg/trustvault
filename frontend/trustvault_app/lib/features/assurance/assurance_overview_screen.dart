...
class AssuranceOverviewScreen extends StatefulWidget {
  ...
  @override
  State<AssuranceOverviewScreen> createState() =>
      _AssuranceOverviewScreenState();
}
...
class _AssuranceOverviewScreenState extends State<AssuranceOverviewScreen> {
  ...
  Future<List<Map<String, dynamic>>> _loadAllEntities() async {
    switch (widget.kind) {
      case AssuranceKind.completeness:
        ...
        final rows = (summary['rows'] as List<dynamic>? ?? <dynamic>[])
        ...
        for (final row in rows) {
          ...
          byEntity.putIfAbsent(
              ...
              () => {
                    ...
                    'status':
                        ((row['missing_count'] as num?)?.toInt() ?? 0) == 0
                            ? 'complete'
                            : 'incomplete',
      ...
      case AssuranceKind.extraction:
        ...
        return (summary['results'] as List<dynamic>? ?? <dynamic>[])
            .cast<Map<String, dynamic>>();
      case AssuranceKind.retention:
        final entities =
            (await _apiClient.getCustomers()).cast<Map<String, dynamic>>();
        final report = await _apiClient.getRetentionReport();
        final reportEntities =
            (report['entities'] as List<dynamic>? ?? <dynamic>[])
                .cast<Map<String, dynamic>>();
        final entityByExternal = {
          for (final entity in entities) '${entity['external_id']}': entity
        };
        return reportEntities.map((entry) {
          ...
          final evidence = (entry['evidence'] as List<dynamic>? ?? <dynamic>[])
              .cast<Map<String, dynamic>>();
          final holds = evidence
              .where((row) =>
                  '${row['legal_hold_status'] ?? 'none'}'.toLowerCase() !=
                  'none')
              .length;
          final deletionEligible =
              evidence.where((row) => row['deletion_eligible'] == true).length;
      ...
      case AssuranceKind.integrity:
        ...
        final results = (summary['results'] as List<dynamic>? ?? <dynamic>[])
            .cast<Map<String, dynamic>>();
        return results.map((result) {
          final entity = entityById['${result['entity_id']}'] ??
              entityByExternal['${result['entity_external_id']}'] ??
              <String, dynamic>{};
          final payloads =
          ...
          final missingHdus =
              (result['missing_required_hdus'] as List<dynamic>? ?? <dynamic>[])
                  .length;
          final errors =
              (result['errors'] as List<dynamic>? ?? <dynamic>[]).length;
          final overall = '${result['overall_status'] ?? 'unknown'}';
          final ok = (overall == 'valid' || overall == 'not_checked') &&
              failed == 0 &&
              missingHdus == 0 &&
              errors == 0;
          final checked =
              result['validated'] == true || result['summary_mode'] == 'full';
          return <String, dynamic>{
            ...entity,
            ...
            'display_name': entity['display_name'] ??
                ...
                result['entity_id'] ??
                '-',
            'status': checked ? overall : 'not checked',
            'score': ok ? 100 : 0,
            'issue_count': failed + missingHdus + errors,
            'summary': checked
                ? '$failed failed payloads · $missingHdus missing HDUs'
                : '${result['evidence_object_count'] ?? 0} payloads · validate in detail',
  ...
  List<Map<String, dynamic>> _retentionRows(Map<String, dynamic> report) {
    ...
    final evidence = (entities.first as Map<String, dynamic>)['evidence']
            as List<dynamic>? ??
        <dynamic>[];
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError)
                return SizedBox(
                    ...
                    child: Center(
                        child: Text(
                            'Unable to load assurance summary: ${snapshot.error}')));
              ...
              return Column(
                  ...
                  children: [
                    ...
                    CustomerSelectorCard(
                        title: 'Search by entity name / ID',
                        subtitle: 'Select an entity to view assurance detail.',
                        onChanged: (_) => _loadDetail()),
                    const SizedBox(height: 12),
                    _DetailPanel(
                        kind: widget.kind,
                        future: _detailFuture,
                        loadedFor: _loadedFor,
                        retentionRows: _retentionRows),
...
class _SummaryCards extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ok = rows
        .where((row) =>
            (row['score'] as num? ?? 0) >= 100 && row['status'] != 'hold')
        .length;
    final avg = rows.isEmpty
        ...
        : (rows
                ...
                rows.length)
            .round();
    final issues = rows.fold<int>(0,
        (total, row) => total + ((row['issue_count'] as num?)?.toInt() ?? 0));
    final statusLabel = kind == AssuranceKind.retention
        ? '$issues legal/retention flags'
        : '$avg%';
    return Wrap(spacing: 12, runSpacing: 12, children: [
      _MetricCard(
          label: kind == AssuranceKind.retention
              ? 'Legal/retention flags'
              : 'Overall status',
          value: statusLabel,
          good: kind == AssuranceKind.retention ? issues == 0 : avg == 100),
      _MetricCard(
          label: 'Entities OK',
          value: '$ok / ${rows.length}',
          good: ok == rows.length),
      _MetricCard(
          label: 'Items requiring review', value: '$issues', good: issues == 0),
    ]);
  }
}
...
class _AllEntitiesGrid extends StatelessWidget {
  const _AllEntitiesGrid(
      {required this.kind, required this.rows, required this.onSelected});
  final AssuranceKind kind;
  final List<Map<String, dynamic>> rows;
  ...
  List<TrustVaultDataGridColumn> _columns() => [
        const TrustVaultDataGridColumn(
            key: 'external_id', label: 'Entity', width: 130),
        const TrustVaultDataGridColumn(
            key: 'display_name', label: 'Name', width: 220),
        const TrustVaultDataGridColumn(
        ...
        const TrustVaultDataGridColumn(
            key: 'issue_count', label: 'Issues', width: 90),
        const TrustVaultDataGridColumn(
            key: 'summary', label: 'Summary', width: 360),
        if (kind == AssuranceKind.extraction) ...const [
          TrustVaultDataGridColumn(
              key: 'indexed_entry_count',
              label: 'Indexed',
              width: 100,
              visibleByDefault: false),
          TrustVaultDataGridColumn(
              key: 'text_row_count', label: 'Text rows', width: 110),
          TrustVaultDataGridColumn(
              key: 'character_count', label: 'Characters', width: 120)
        ],
        if (kind == AssuranceKind.integrity) ...const [
          TrustVaultDataGridColumn(
              key: 'container_version_id',
              label: 'Container',
              width: 260,
              visibleByDefault: false),
          TrustVaultDataGridColumn(
              key: 'fits_opened', label: 'FITS opened', width: 110),
          TrustVaultDataGridColumn(
              key: 'container_hash_matches', label: 'Hash matches', width: 130)
        ],
      ];
}
...
class _DetailPanel extends StatelessWidget {
  const _DetailPanel(
      ...
      required this.loadedFor,
      required this.retentionRows});
  final AssuranceKind kind;
  ...
  Widget build(BuildContext context) {
    if (future == null)
      return const SizedBox(
          height: 260,
          child: Center(child: Text('Select an entity to view detail.')));
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError)
          return SizedBox(
              height: 360,
              child: Center(
                  child: Text('Unable to load detail: ${snapshot.error}')));
        final data = snapshot.data ?? <String, dynamic>{};
        switch (kind) {
          case AssuranceKind.completeness:
            return _CompletenessTable(
                rows: (data['results'] as List<dynamic>? ?? <dynamic>[])
                    .cast<Map<String, dynamic>>());
          case AssuranceKind.extraction:
            return _ExtractionTable(
                rows: (data['ocr_text'] as List<dynamic>? ?? <dynamic>[])
                    .cast<Map<String, dynamic>>());
          case AssuranceKind.retention:
...
class _CompletenessTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) => TrustVaultDataGrid(
      ...
      columns: const [
        ...
        TrustVaultDataGridColumn(key: 'rule_key', label: 'Rule', width: 220),
        TrustVaultDataGridColumn(
      ...
      exportFilename: 'trustvault-completeness-detail.csv',
      height: 420,
      dense: true);
}
...
class _ExtractionTable extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => TrustVaultDataGrid(
      title: 'Extraction detail',
      rows: rows
          .map((row) =>
              {...row, 'preview': _preview('${row['extracted_text'] ?? ''}')})
          .toList(),
      columns: const [
        TrustVaultDataGridColumn(
            key: 'filename', label: 'Filename', width: 260),
        TrustVaultDataGridColumn(
            key: 'extraction_method', label: 'Method', width: 140),
        TrustVaultDataGridColumn(
            key: 'extraction_confidence', label: 'Confidence', width: 110),
        TrustVaultDataGridColumn(
            key: 'character_count', label: 'Characters', width: 110),
        TrustVaultDataGridColumn(
            key: 'preview', label: 'Text preview', width: 520),
        TrustVaultDataGridColumn(
            key: 'object_id',
            label: 'Object',
            width: 300,
      ...
      exportFilename: 'trustvault-extraction-detail.csv',
      height: 420,
      dense: true);
}
...
class _RetentionTable extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => TrustVaultDataGrid(
      title: 'Legal hold & retention detail',
      rows: rows,
      columns: const [
        TrustVaultDataGridColumn(
            key: 'filename', label: 'Filename', width: 260),
        ...
        TrustVaultDataGridColumn(
            key: 'document_type', label: 'Document type', width: 180),
        TrustVaultDataGridColumn(
            key: 'retention_class', label: 'Retention class', width: 170),
        TrustVaultDataGridColumn(
            key: 'retention_until', label: 'Retention until', width: 150),
        TrustVaultDataGridColumn(
            key: 'legal_hold_status', label: 'Legal hold', width: 130),
        TrustVaultDataGridColumn(
            key: 'deletion_eligible', label: 'Deletion eligible', width: 150),
        TrustVaultDataGridColumn(
            key: 'object_id',
            label: 'Object',
            width: 300,
      ...
      exportFilename: 'trustvault-retention-detail.csv',
      height: 420,
      dense: true);
}
...
class _IntegrityTable extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final payloads = (data['payload_results'] as List<dynamic>? ??
            data['payloads'] as List<dynamic>? ??
            <dynamic>[])
        .cast<Map<String, dynamic>>();
    final failed = payloads
        .where((row) => row['status'] != 'valid' && row['valid'] != true)
        .toList();
    ...
    return TrustVaultDataGrid(
        ...
        columns: const [
          TrustVaultDataGridColumn(
              key: 'hdu_name', label: 'Payload', width: 200),
          TrustVaultDataGridColumn(
              key: 'payload_hdu',
              label: 'Payload HDU',
              width: 200,
          ...
          TrustVaultDataGridColumn(
              key: 'filename', label: 'Filename', width: 260),
          TrustVaultDataGridColumn(key: 'status', label: 'Status', width: 120),
          TrustVaultDataGridColumn(key: 'valid', label: 'Valid', width: 100),
          TrustVaultDataGridColumn(
              key: 'expected_sha256', label: 'Expected SHA-256', width: 320),
          TrustVaultDataGridColumn(
              key: 'actual_sha256', label: 'Actual SHA-256', width: 320),
          TrustVaultDataGridColumn(
              key: 'sha256',
              label: 'SHA-256',
              width: 320,
        ...
        exportFilename: 'trustvault-integrity-detail.csv',
        height: 420,
        dense: true);
  }
}
...
String _preview(String text) =>
    text.length <= 280 ? text : '${text.substring(0, 280)}...';
