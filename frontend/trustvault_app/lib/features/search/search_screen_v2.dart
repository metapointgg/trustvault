...
import 'search_result_panel.dart';
...
class _SearchScreenV2State extends State<SearchScreenV2> {
  ...
  String _inferIndustryFromText(String value) {
    ...
    if (lower.contains('entity') || lower.contains('customer') || lower.contains('client') || lower.contains('cdd') || lower.contains('source of funds') || lower.contains('screening') || lower.contains('onboarding')) return 'financial_services';
    return 'auto';
  }
  ...
  Future<void> _openRow(Map<String, dynamic> row) async {
    ...
    if (externalId.isEmpty || externalId == 'null' || externalId == 'ARCHIVE') return;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const Text('Ask TrustVault a question, choose the industry context for this search, and review the result in the most appropriate format.'),
              ]),
            ),
            OutlinedButton.icon(onPressed: () => setState(_reloadLookups), icon: const Icon(Icons.refresh), label: const Text('Refresh')),
          ]),
          ...
          if (_loading)
            ...
            SearchResultPanel(response: _response!, onOpenRow: _openRow, onPreview: _openEvidence, onJson: _showJson),
