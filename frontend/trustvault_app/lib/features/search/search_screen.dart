...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    _entityFilterController.addListener(() => setState(() =>
        _entityFilter = _entityFilterController.text.trim().toLowerCase()));
  }
  ...
  Future<void> _run() async {
    ...
    try {
      final result = await _apiClient.executeQuery(
        query: query,
        entityExternalId:
            _selectedEntityOnly && _entityController.text.trim().isNotEmpty
  ...
  void _applyExample(String example) {
    ...
    setState(() {
      ...
      _selectedEntityOnly = cleaned.contains('CUST-000001') ||
          cleaned.toLowerCase().contains('selected entity');
      if (cleaned.contains('CUST-000001'))
    ...
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
            width: 920,
            height: 700,
            child: _EvidencePreview(apiClient: _apiClient, preview: preview)),
        actions: [
          TextButton.icon(
              onPressed: () => launchUrl(
                  Uri.parse(_apiClient.evidenceFileUrl(objectId)),
                  webOnlyWindowName: '_blank'),
              icon: const Icon(Icons.open_in_new),
              label: const Text('Open')),
          TextButton.icon(
              onPressed: () => launchUrl(
                  Uri.parse(_apiClient.evidenceDownloadUrl(objectId)),
                  webOnlyWindowName: '_blank'),
  ...
  void _showJson(String title, Object value) {
    showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        title: Text(title),
        content:
            SizedBox(width: 960, height: 680, child: _JsonPanel(value: value)),
        actions: [
          TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text('Close'))
  ...
  List<Map<String, dynamic>> _filteredEntities(List<dynamic> rows) {
    ...
    return entities
        .where((row) =>
            '${row['external_id']} ${row['display_name']} ${row['risk_rating']} ${row['jurisdiction']}'
                .toLowerCase()
                .contains(_entityFilter))
        .take(20)
        .toList();
  }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(
                child: Column(
                  ...
                  const Text(
                      'Ask TrustVault a question and review evidence rows. Click evidence rows to preview them, or entity rows to open the entity context.'),
                ])),
            OutlinedButton.icon(
                onPressed: () => setState(() {
                      _scenariosFuture = _apiClient.getQueryScenarios();
          ...
          _SearchForm(
              ...
              onIncludeAiSummaryChanged: (value) =>
                    ...
                    SizedBox(
                        ...
                        child: SegmentedButton<bool>(
                            ...
                            onSelectionChanged: (value) =>
                                onSelectedEntityOnlyChanged(value.first))),
                    SizedBox(
                        width: 260,
                        child: TextField(
                            controller: entityController,
                            enabled: selectedEntityOnly,
                            decoration: const InputDecoration(
                                labelText: 'Selected entity ID',
                                border: OutlineInputBorder()))),
                  ]),
              SwitchListTile(
                  ...
                  subtitle: const Text(
                      'Shows a short narrative above the grid. The evidence grid remains the primary result.'),
  ...
  Widget build(BuildContext context) => OutlinedButton.icon(
      ...
      onPressed: () async {
        ...
        final selected = await showDialog<String>(
            context: context, builder: (_) => _ExamplesDialog(data: data));
...
class _ExamplesDialogState extends State<_ExamplesDialog> {
  ...
  void initState() {
    ...
    _controller.addListener(
        () => setState(() => _filter = _controller.text.trim().toLowerCase()));
  }
  ...
  Widget build(BuildContext context) {
    final scenarios =
        (widget.data['scenarios'] as List<dynamic>? ?? <dynamic>[])
            .whereType<Map<String, dynamic>>()
            .toList();
    final groups = <String>[
      'All',
      ...scenarios.map((item) => '${item['group']}')
    ];
    ...
    for (final scenario in scenarios) {
      ...
      for (final item
          in (scenario['examples'] as List<dynamic>? ?? <dynamic>[])) {
        final text = '$item'
            .replaceAll('customers', 'entities')
            .replaceAll('customer', 'entity')
            .replaceAll('clients', 'entities')
            .replaceAll('client', 'entity');
        if (_filter.isEmpty || '$group $text'.toLowerCase().contains(_filter))
          examples.add({'group': group, 'text': text});
      }
    }
    return AlertDialog(
          ...
          child:
            ...
            TextField(
                ...
                decoration: const InputDecoration(
                    ...
                    prefixIcon: Icon(Icons.search),
                    labelText:
                        'Search by entity name, ID, risk or jurisdiction')),
            const SizedBox(height: 12),
            FutureBuilder<List<dynamic>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError)
                    return Text('Unable to load entities: ${snapshot.error}');
                      ...
                      child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                                dense: true,
                                title: Text(
                                    '${row['external_id']} · ${row['display_name']}'),
                                subtitle: Text(
                                    'Risk: ${row['risk_rating'] ?? '-'} · Jurisdiction: ${row['jurisdiction'] ?? '-'}'),
                                trailing: TextButton(
                                    onPressed: () => onSelected(row),
...
class _ResultPanelState extends State<_ResultPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final rows = (result['results'] as List<dynamic>? ?? <dynamic>[])
        ...
        .toList();
    final rawSummary =
        '${(widget.response['ai_summary'] as Map<String, dynamic>?)?['summary'] ?? ''}';
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Wrap(spacing: 8, runSpacing: 8, children: [
        ...
        Chip(
            label:
                Text('Source: ${widget.response['execution_source'] ?? '-'}')),
      ...
      Wrap(spacing: 8, children: [
        OutlinedButton(
            onPressed: () => widget.onJson(
                'Structured query', widget.response['structured_query'] ?? {}),
            child: const Text('Structured query')),
        OutlinedButton(
            onPressed: () => widget.onJson(
                'Interpretation', widget.response['interpretation'] ?? {}),
            child: const Text('Interpretation')),
        OutlinedButton(
            onPressed: () =>
                widget.onJson('Diagnostics', result['diagnostics'] ?? {}),
            child: const Text('Diagnostics')),
        OutlinedButton(
            onPressed: () => widget.onJson('Raw JSON', widget.response),
  ...
  Map<String, dynamic> _flattenRow(Map<String, dynamic> row) {
    final metadata =
        row['metadata'] as Map<String, dynamic>? ?? <String, dynamic>{};
    final nested =
        metadata['metadata'] as Map<String, dynamic>? ?? <String, dynamic>{};
    return <String, dynamic>{
      ...
      'retention_class': row['retention_class'] ??
          ...
          nested['retention_class'],
      'legal_hold_status': row['legal_hold_status'] ??
          metadata['legal_hold_status'] ??
          nested['legal_hold_status'],
      'snippet': row['snippet'] ??
          row['text_content'] ??
          row['status'] ??
          row['summary_type'] ??
  ...
  List<TrustVaultDataGridColumn> _columns() => [
        const TrustVaultDataGridColumn(
            key: 'entity_external_id', label: 'Entity', width: 130),
        const TrustVaultDataGridColumn(
            key: 'entity_display_name', label: 'Name', width: 220),
        const TrustVaultDataGridColumn(
            key: 'risk_rating', label: 'Risk', width: 90),
        const TrustVaultDataGridColumn(
            key: 'jurisdiction', label: 'Jurisdiction', width: 130),
        const TrustVaultDataGridColumn(
        ...
        const TrustVaultDataGridColumn(
            key: 'source_system', label: 'Source', width: 160),
        const TrustVaultDataGridColumn(
            key: 'match_score', label: 'Score', width: 80),
        const TrustVaultDataGridColumn(
            key: 'status',
            label: 'Status',
            width: 120,
        ...
        const TrustVaultDataGridColumn(
            ...
            visibleByDefault: false),
        const TrustVaultDataGridColumn(
            key: 'snippet', label: 'Snippet / status', width: 520),
      ];
...
class _NarrativeSummary extends StatelessWidget {
  const _NarrativeSummary(
      {required this.summary, required this.expanded, required this.onToggle});
    ...
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(
                child: Text('AI narrative summary',
                    style: Theme.of(context)
                        .textTheme
                        .titleSmall
                        ?.copyWith(fontWeight: FontWeight.w700))),
          ...
          AnimatedContainer(
              ...
              child: Scrollbar(
                  ...
                  child:
                      SingleChildScrollView(child: SelectableText(summary)))),
        ]));
  }
}
      ...
      child: Center(
          child: Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(height: 8),
                const Text(
                    'Ask a question, select all entities or one entity, then run Search.')
              ]))));
}
