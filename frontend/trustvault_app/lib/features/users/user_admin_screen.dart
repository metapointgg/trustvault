...
class _UserAdminScreenState extends State<UserAdminScreen> {
  ...
  Future<void> _showCreateDialog(List<String> roles) async {
    ...
    final selectedRoles = <String>{
      roles.contains('Read-only Auditor') ? 'Read-only Auditor' : roles.first
    };
              ...
              content: SizedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    const Text(
                        'The user is created in the local database with the initial verifier supplied here. Rotate it through the controlled operational process for this deployment.'),
                  ],
                ),
              ),
              actions: [
  ...
  Future<void> _showEditDialog(
      Map<String, dynamic> user, List<String> roles) async {
    final displayNameController =
        TextEditingController(text: '${user['display_name'] ?? ''}');
    final selectedRoles = ((user['roles'] as List<dynamic>? ?? <dynamic>[])
        .map((role) => '$role')).toSet();
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: FutureBuilder<Map<String, dynamic>>(
        future: Future.wait([_usersFuture, _rolesFuture]).then((values) =>
            <String, dynamic>{'users': values[0], 'roles': values[1]}),
        builder: (context, snapshot) {
          ...
          final usersResponse =
              snapshot.data?['users'] as Map<String, dynamic>? ??
                  <String, dynamic>{};
          final rolesResponse =
              snapshot.data?['roles'] as Map<String, dynamic>? ??
                  <String, dynamic>{};
          final users =
              (usersResponse['users'] as List<dynamic>? ?? <dynamic>[])
                  .cast<Map<String, dynamic>>();
          final roles =
              (rolesResponse['roles'] as List<dynamic>? ?? <dynamic>[])
                  .map((role) => '$role')
                  .toList();
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                            'Manage local TrustVault users and role-based access controls.'),
                      ],
                    ),
                  ),
                  OutlinedButton.icon(
                  ...
                  FilledButton.icon(
                      onPressed: () => _showCreateDialog(roles),
