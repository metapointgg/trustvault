...
class _EntitiesScreenState extends State<EntitiesScreen> {
  ...
  Future<void> _validateContainerVersion(Map<String, dynamic> version) async {
    ...
    showDialog<void>(
      context: context,
      builder: (context) {
        final payloadResults = (result['payload_results'] as List<dynamic>? ?? <dynamic>[]).cast<Map<String, dynamic>>();
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Expanded(
                child: TrustVaultDataGrid(
                  ...
                  rows: payloadResults,
                  columns: const [
                    TrustVaultDataGridColumn(key: 'hdu_name', label: 'HDU', width: 180),
                    TrustVaultDataGridColumn(key: 'filename', label: 'Filename', width: 260),
                    TrustVaultDataGridColumn(key: 'valid', label: 'Valid', width: 90),
                    TrustVaultDataGridColumn(key: 'expected_sha256', label: 'Expected SHA-256', width: 320),
                    TrustVaultDataGridColumn(key: 'actual_sha256', label: 'Actual SHA-256', width: 320),
                    TrustVaultDataGridColumn(key: 'header_sha256', label: 'Header SHA-256', width: 320, visibleByDefault: false),
                  ],
  ...
  Future<void> _showEvidence(Map<String, dynamic> entity) async {
    ...
    final rows = evidenceObjects.cast<Map<String, dynamic>>().map((evidence) {
      ...
      return <String, dynamic>{
        ...evidence,
        'filename': metadata['filename'] ?? metadata['original_filename'] ?? evidence['storage_uri'] ?? '-',
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: TrustVaultDataGrid(
            ...
            columns: const [
              TrustVaultDataGridColumn(key: 'filename', label: 'Filename', width: 260),
              TrustVaultDataGridColumn(key: 'object_type', label: 'Type', width: 140),
              ...
              TrustVaultDataGridColumn(key: 'source_system', label: 'Source', width: 180),
              TrustVaultDataGridColumn(key: 'content_type', label: 'Content type', width: 160),
  ...
  Future<void> _open(String url) async {
    await showDialog<void>(context: context, builder: (context) => AlertDialog(title: const Text('Open evidence'), content: SelectableText(url), actions: [TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Close'))]));
  }
        ...
        content: SizedBox(
          ...
          child: TrustVaultDataGrid(
            ...
            columns: const [
              ...
              TrustVaultDataGridColumn(key: 'evidence_object_count', label: 'Evidence', width: 100),
              TrustVaultDataGridColumn(key: 'size_bytes', label: 'Size bytes', width: 120),
              TrustVaultDataGridColumn(key: 'storage_uri', label: 'Storage URI', width: 460),
  ...
  List<TrustVaultDataGridColumn> _columns() => [
    const TrustVaultDataGridColumn(key: 'external_id', label: 'Entity ID', width: 130),
    const TrustVaultDataGridColumn(key: 'display_name', label: 'Name', width: 240),
    ...
    const TrustVaultDataGridColumn(key: 'entity_type', label: 'Type', width: 130, visibleByDefault: false),
    const TrustVaultDataGridColumn(key: 'status', label: 'Status', width: 110, visibleByDefault: false),
    const TrustVaultDataGridColumn(key: 'evidence_object_count', label: 'Evidence', width: 100),
    TrustVaultDataGridColumn(key: 'has_current_fits_container', label: 'Current FITS', width: 120, cellBuilder: (row) => _StatusPill(label: row['has_current_fits_container'] == true ? 'Yes' : 'No', positive: row['has_current_fits_container'] == true)),
    const TrustVaultDataGridColumn(key: 'current_container_version_number', label: 'Version', width: 100),
    TrustVaultDataGridColumn(key: 'actions', label: 'Actions', width: 70, sortable: false, valueBuilder: (_) => '', cellBuilder: (row) => PopupMenuButton<String>(tooltip: 'Actions', onSelected: (value) => _handleAction(row, value), itemBuilder: (context) => const [PopupMenuItem(value: 'evidence', child: Text('View evidence')), PopupMenuItem(value: 'versions', child: Text('FITS versions')), PopupMenuItem(value: 'search', child: Text('Search this entity')), PopupMenuItem(value: 'completeness', child: Text('Completeness')), PopupMenuDivider(), PopupMenuItem(value: 'rebuild', child: Text('Rebuild FITS now')), PopupMenuItem(value: 'queue', child: Text('Queue rebuild'))])),
  ];
  ...
  Widget build(BuildContext context) {
    return Scrollbar(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Entities', style: Theme.of(context).textTheme.displaySmall?.copyWith(fontWeight: FontWeight.w700)), const SizedBox(height: 8), const Text('Entity records and their current FITS evidence archive lifecycle. Click a row to view the evidence list.')])), OutlinedButton.icon(onPressed: () => setState(() => _future = _apiClient.getCustomers()), icon: const Icon(Icons.refresh), label: const Text('Refresh'))]),
          ...
          FutureBuilder<List<dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) return SizedBox(height: 520, child: Center(child: Text('Unable to load entities: ${snapshot.error}')));
              final rows = (snapshot.data ?? <dynamic>[]).cast<Map<String, dynamic>>();
              return TrustVaultDataGrid(title: 'Entities', subtitle: 'Search, sort, show/hide columns and export entity records.', rows: rows, columns: _columns(), initialSortColumnKey: 'external_id', onRowTap: _showEvidence, exportFilename: 'trustvault-entities.csv', emptyText: 'No matching entities.', height: 640, dense: true);
            },
          ),
...
class _StatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(borderRadius: BorderRadius.circular(999), color: positive ? scheme.primaryContainer : scheme.errorContainer), child: Text(label, style: TextStyle(color: positive ? scheme.onPrimaryContainer : scheme.onErrorContainer)));
  }
}
...
class _DetailRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [SizedBox(width: 140, child: Text(label, style: const TextStyle(fontWeight: FontWeight.w700))), Expanded(child: SelectableText(value))]));
  }
}
