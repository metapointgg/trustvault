...
The intended FITS contract should explicitly preserve the following logical sections where available:
...
This ensures that TrustVault preserves not only the original evidence payloads, but also the extracted search text, structured extracted intelligence, extraction provenance and fixity information needed to validate and interpret the archive in the future.
...
### Event-driven remediation ingestion

TrustVault should retain the event-driven remediation pattern proven in the proof of concept. This is distinct from source-folder ZIP ingestion and supports operational updates from source systems after the historic archive has been loaded.

Typical event-driven flow:

```text
source system event
  -> event payload identifies entity and evidence payload
  -> worker ingests the new evidence
  -> affected entity FITS archive is rebuilt
  -> FITS-derived index projection is updated for that entity
  -> completeness/search/audit views reflect the remediation
```
...
```json
{
  ...
  "entity_id": "NTC-0002",
  "payload_path": "incoming_payloads/NTC-0002_new_proof_of_address.pdf",
  "category": "Address",
  "document_type": "Proof of Address",
  "source_system": "Salesforce FSC",
  "snapshot_id": "CDD_REMEDIATION_2026",
  "snapshot_type": "CDD Remediation",
  "captured_at": "2026-05-16T09:15:00Z"
}
```

This pattern is important because it demonstrates that TrustVault is not only a static legacy archive. It can receive ongoing evidence updates, remediate completeness gaps and preserve the resulting entity archive version.

Production requirements for this area:

- support event JSON ingestion in addition to ZIP source-folder ingestion;
- resolve payload paths safely relative to configured inbound locations;
- rebuild only the affected entity FITS archive where possible;
- update only the affected entity index projection where possible;
- record job and audit events with a shared correlation ID;
- support failed-event replay and quarantine;
- expose remediation impact in completeness/search views.
...
Natural-language search should be converted into a controlled structured query before execution. The execution pipeline should be:
...
raw user query
  ...
  -> direct FITS / PostgreSQL index / completeness execution
  -> result grid
  -> optional AI summary from retrieved rows only
```

The AI layer must never execute SQL or directly search the archive. It may assist with interpretation and summarisation, but deterministic TrustVault logic controls the execution route and evidence retrieval.

### Required query normalisation rules

The production interpreter must normalise local AI output before execution. In particular:

- `ONBOARDING` is a lifecycle/snapshot stage, not a document type;
- onboarding, onboarding documentation, onboarding documents, customer onboarding and account opening should map to `snapshot_id = "ONBOARDING"`;
- `document_type = "ONBOARDING"` must never be executed;
- invented document types should be cleared or mapped to controlled values before execution;
- whole natural-language questions should not be used as keyword filters;
- missing-evidence queries must use evidence identity fields, not OCR/search text.

Controlled document types should include:
...
```json
{
  "raw_query": "Show me onboarding documentation for customers in Guernsey who are high risk",
...
Incorrect interpretation that must be normalised or rejected:
...
### Missing-evidence execution rule
...
TrustVault should use deterministic `NOT EXISTS`-style logic against evidence identity fields such as document type, category, filename and snapshot. It should not treat mentions in OCR/search text as evidence presence. A CDD review that says “proof of address is missing” must not be mistaken for proof-of-address evidence.

### Search explainability

Advanced/admin mode should expose:

- raw query;
- deterministic fallback interpretation;
- AI interpretation where used;
- normalised structured query;
- execution route;
- filters applied;
- keyword terms applied;
- row count before/after filter stages where available;
- search source used: direct FITS, PostgreSQL index, vector index or fallback;
- warnings for relaxed or removed filters.
...
### AI summary input contract

AI summaries must only be generated after deterministic retrieval. The model should receive a compact, deduplicated digest of retrieved rows, not raw database rows, large OCR dumps or binary payloads.

Recommended summary digest shape:

```json
{
  "query": "Show me source of funds evidence for high-risk entities",
  "result_scope": {
    "total_rows": 50,
    "total_entities": 17,
    "summarised_entities": 8
  },
  "entities": [
    {
      "entity_id": "NTC-0002",
      "display_name": "Oliver Hartley",
      "risk_rating": "High",
      "jurisdiction": "Guernsey",
      "evidence": [
        {
          "object_id": "...",
          "document_type": "Source of Funds",
          "filename": "source_of_funds.pdf",
  ...
  "caveats": [
    ...
    "The preserved FITS archive remains the source of truth."
  ]
}
```

AI summary preparation rules:

- deduplicate rows before summary;
- group by authoritative entity ID;
- use display names from entity metadata/index fields, not OCR text;
- do not infer alternative identities from OCR text;
- cap the number of entities and evidence items sent to the model;
- use short snippets only;
- include the total result count and summarised subset count;
- include caveats about OCR and the FITS archive being authoritative;
- cache summaries by result-set hash so row selection does not regenerate them.

Production summaries should be stored or cached with:
...
The Completeness screen should support both an entity-level view and a rule-level view.

The entity-level view should show one row per evaluated entity, including:
...
The rule-level view should show one row per rule/entity combination, including:
...
Selecting an entity-level row should open the detailed checklist and the matched evidence. This mirrors the proof-of-concept behaviour and gives compliance operators a clearer operational picture than a rule-level grid alone.
...
TrustVault exports preserved FITS evidence archives and should also support regulator-ready evidence packs derived from selected evidence, queries, completeness results or entity records.

The FITS file remains the primary archive artefact. Derived export packs are presentation and response artefacts that make the preserved evidence easier to review, approve or provide to a regulator.
...
Production export types should include:

1. FITS archive download;
2. entity CDD evidence pack;
3. query result evidence pack;
4. completeness remediation pack;
5. integrity attestation pack;
6. regulator response pack.

A regulator-ready evidence pack should include, where applicable:
...
AUDIT_CONTEXT.json
files/ or FITS archive reference
```

Production export controls should include:

- role-based export permissions;
- audit event for export request, approval and download;
- optional export approval workflow;
- temporary secure download links;
- signed export attestations;
- clear distinction between the FITS archive and derived presentation material;
- ability to include the AI summary only where generated and permitted.
...
### Deployment pack requirement

For the first controlled deployment, TrustVault should be packaged as a single-tenant deployment pack that can be deployed into the client’s AWS account or Azure subscription.

The deployment pack should include:
...
deployment/
  ...
  runbooks/
    ...
    operations.md
  examples/
    licence.example.json
    environment.example.env
```

Minimum production deployment behaviour:

- API and worker run as separate containers;
- FITS archives and exports are stored outside the container filesystem;
- PostgreSQL stores operational state and rebuildable projections;
- queues are used for ingestion, rebuild, export and integrity jobs;
- secrets are provided by Secrets Manager or Key Vault;
- object storage is private by default;
- outbound AI/OCR calls are disabled unless configured;
- all material actions produce audit events.
...
## 33. Pre-deployment readiness priorities

Before the first controlled client deployment, the following items should be treated as release gates rather than general roadmap items.

### P0 — required before a real client pilot

- Query interpretation normalisation and regression tests.
- Evidence pack export beyond simple FITS download.
- Signed licence validation and module enforcement.
- Role checks around evidence preview, export, ingestion and administration.
- Audit coverage for search, preview, export, ingestion, jobs and licence events.
- Clear deployment pack for at least one cloud target.
- Server-side pagination/filtering for large grids, or explicit pilot volume limits.
- Clear distinction in the UI/API between FITS archive, PostgreSQL projection, AI summary and derived export pack.

### P1 — should be completed early in pilot

- Real OCR/extraction provider workflow.
- Legal hold placement/removal workflow.
- Scheduled integrity validation and exception reporting.
- Job retry, cancel, progress and worker heartbeat visibility.
- Export approval workflow.
- Improved PDF/email preview.
- Tamper-evident audit strategy.

### P2 — later hardening

- Saved grid views and user-specific column preferences.
- Ruleset lifecycle: draft, active, superseded and archived.
- Historical completeness trend reporting.
- Full external IdP integration.
- Malware scanning/quarantine pipeline.
- AWS/Azure Marketplace packaging.
- MCP server for safe local AI access to TrustVault tools.

---

## 34. Known incomplete developments and roadmap
...
### MCP server for local AI clients

A future MCP server should allow LM Studio or other MCP-compatible local AI clients to query TrustVault through controlled tools. This should be read-only by default and must not expose arbitrary SQL, arbitrary filesystem reads or unrestricted payload download.
...
- list entities;
- get entity summary;
- search selected entity FITS directly;
- search archive index projection;
- interpret query;
- execute query;
- check completeness;
- return evidence payload metadata;
- export evidence pack only when explicitly enabled.

Default MCP safety posture:

- read-only by default;
- no raw binary payload return by default;
- no export by default;
- configured archive paths only;
- audit-ready tool calls;
- evidence snippets and metadata only unless explicitly enabled.
